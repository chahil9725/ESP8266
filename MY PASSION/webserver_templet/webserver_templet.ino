...
void Config()
{
  server.on("/", []() {
p=0;
    int carrier = server.arg("carrier").toInt();
    String input1 = server.arg("code");
    ...
    Serial.print(carrier);
    int input1_len = input1.length() + 1;
    char input[input1_len];
    input1.toCharArray(input, input1_len);
    char separator[] = " ";
    char *token;
    int le = 0;
    uint16_t buff[500];
    ...
    irsend.sendRaw(buff, le, carrier);  // Send a raw data capture at 38kHz.
#endif
